...
def process_anyres_image_video(image, processor, grid_pinpoints):
    """
    Process an image with variable resolutions.
...
def process_images(images, image_processor, model_cfg, video=False):
    ...
    if image_aspect_ratio == 'pad':
        ...
        for image in images:
            if video:
                image = process_anyres_image_video(image, image_processor, model_cfg.image_grid_pinpoints)
            else:
                image = process_anyres_image(image, image_processor, model_cfg.image_grid_pinpoints)
            new_images.append(image)
    else:
        return image_processor(images, return_tensors='pt')['pixel_values']
    if not video and all(x.shape == new_images[0].shape for x in new_images):
